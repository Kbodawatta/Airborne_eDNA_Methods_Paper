...
Data <- read.csv2("BirT_Meth_Stat_All.csv", row.names = 1, check.names = FALSE)

#Testing the effect of filter types on species richness using linear mixed models
model1 <- lmer(Richness ~  Storage_type + (1|Sampler) , Data)
...
ggplot(Data, aes(x=Storage_type, y=Richness)) + geom_boxplot(aes(fill = Storage_type), outlier.shape = NA)+ geom_point(aes(size = 7))+ scale_fill_manual(values=c("#91de45" ,"#bc4ec2","#63d6e6")) + scale_color_manual(c("#000")) +theme_classic()+ geom_line(aes(group=Sampler), linetype = 2)

###Generating Upset plots
library(UpSetR)
Up <- read.csv2("BirT_Upset.csv", row.names = 1, check.names = FALSE)
Up <- as.data.frame(Up)
...
#All other samplers
Tab5 <- read.csv2("BirT_Meth_Others.csv", row.names = 1, check.names = FALSE)
Tab6 <- Tab5[3:31]
sp1 <- specaccum(Tab6)
sp2 <- specaccum(Tab6, "random")
sp2
summary(sp2)
plot(sp1, ci.type="poly", col="blue", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0, 30), ylim=c(0, 40))
boxplot(sp2, col="yellow", add=TRUE, pch="+")


##only for best sampler
Tab3 <- read.csv2("BirT_Meth_Dry.csv", row.names = 1, check.names = FALSE)
Tab4 <- Tab3[3:38]
sp1 <- specaccum(Tab4)
sp2 <- specaccum(Tab4, "random")
sp2
summary(sp2)
plot(sp1, ci.type="poly", col="red", lwd=2, ci.lty=0, ci.col="lightgray", xlim=c(0, 30), ylim=c(0, 40))
...
Tab <- read.csv2("BirT_Meth_OTU.csv", row.names = 1, check.names = FALSE)
...
#download the coordinates and then add coordinates to the BirT_Meth_Stat document
write.csv2(princoor.pathotype.data, file = "BirT_Method_Cor_Raup.csv")
...
Data <- read.csv2("BirT_Meth_Stat.csv", row.names = 1, check.names = FALSE)
Data$Storage_type <- factor(Data$Storage_type, levels = c("Frozen", "LB", "Buffer"))

#Plot the PCOA plot
ggplot(data = Data, aes(x = Cor_X, y = Cor_Y)) + geom_point(size=7, aes(colour = Storage_type))+ scale_color_manual(values=c("#91de45" ,"#bc4ec2","#63d6e6")) + theme_classic() + scale_shape_manual(values=c(16, 17, 15)) + geom_line(aes(group=Sampler))+ stat_ellipse(aes(group = Storage_type, col = Storage_type), linetype = 2, type = "t", lwd = 1.2, level = 0.95)
...
species_sf <- st_as_sf(LB, coords = c("Longitude", "Latitude"), crs = 4326)
...
Frozen_OTU <-read.csv2("Mantel_OTU_Frozen.csv", row.names = 1, check.names = FALSE)
LB_OTU<-read.csv2("Mantel_OTU_LB.csv", row.names = 1, check.names = FALSE)
...
Man<- mantel(Frozen_com_dist, Frozen_m, method="pearson", permutations=10000, strata = NULL,na.rm = FALSE)
Man
