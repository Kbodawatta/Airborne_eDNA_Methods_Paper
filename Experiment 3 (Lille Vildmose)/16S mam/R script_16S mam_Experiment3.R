...
Data <- read.csv2("16S_Airflow_All.csv", row.names = 1, check.names = FALSE)
...
#All other samplers
Tab5 <- read.csv2("16S_AirFlow_Others.csv", row.names = 1, check.names = FALSE)
Tab6 <- Tab5[5:17]
sp1 <- specaccum(Tab6)
sp2 <- specaccum(Tab6, "random")
sp2
summary(sp2)
plot(sp1, ci.type="poly", col="blue", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0, 50), ylim=c(0, 16))
boxplot(sp2, col="yellow", add=TRUE, pch="+")

##only for best sampler
Tab3 <- read.csv2("16S_AirFlow_12v.csv", row.names = 1, check.names = FALSE)
Tab4 <- Tab3[5:14]
sp1 <- specaccum(Tab4)
sp2 <- specaccum(Tab4, "random")
sp2
summary(sp2)
plot(sp1, ci.type="poly", col="red", lwd=2, ci.lty=0, ci.col="lightgray", xlim=c(0, 50), ylim=c(0, 16))
...
Tab <- read.csv2("16S_AirFlow_OTU.csv", row.names = 1, check.names = FALSE)
...
write.csv2(princoor.pathotype.data, file = "16S_Airflow_Stat.csv")
...
pairwise.adonis(dis, Meta$SampleSite,  p.adjust.m = "bonferroni", reduce = NULL,
                perm = 10000)

#Now bring in the stat document with the PCOA coordinates
Data <- read.csv2("16S_Airflow_Stat.csv", row.names = 1, check.names = FALSE)
Data$SampleType <- factor(Data$SampleType, levels = c("12v", "12v_5", "5v", "5v_12", "12vP"))

#Plot the PCOA plot
ggplot(data = Data, aes(x = Cord_X, y = Cord_Y)) + geom_point(size=7, aes(colour = SampleType, shape = Date))+ scale_color_manual(values=c("#240da3" ,"#9382ed","#7a0a0f", "#e34f56", "#0a0001")) + theme_classic() + scale_shape_manual(values=c(16, 17, 15)) + geom_line(aes(group=SampleSite))
ggplot(data = Data, aes(x = Cord_X, y = Cord_Y)) + geom_point(size=7, aes(colour = SampleType, shape = Date))+ scale_color_manual(values=c("#240da3" ,"#9382ed","#7a0a0f", "#e34f56", "#0a0001")) + theme_classic() + scale_shape_manual(values=c(16, 17, 15)) + stat_ellipse(aes(group = SampleType, col = SampleType), linetype = 2, type = "t", level = 0.80)
